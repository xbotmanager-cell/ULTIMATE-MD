  ...
  execute: async (sock, msg, args) => {
      try {
         ...
         if ('country' === 'movie') {
             ...
             apiSources.push({ url: `https://api.jikan.moe/v4/country?q=${encodeURIComponent(query)}&limit=1`, pic: 'data.0.images.jpg.image_url', title: 'data.0.title', desc: 'data.0.synopsis', extra: 'data.0.score' });
         } else {
             for(let i=1; i<=20; i++) {
                 apiSources.push({ url: `https://api${i}.example.com/search/country?q=${encodeURIComponent(query)}`, pic: 'thumbnail', title: 'title', desc: 'description', extra: 'info' });
             }
         }
         ...
         await sock.sendMessage(msg.key.remoteJid, { text: 'Error in search country.' }, { quoted: msg });
      }
  }
};
