  ...
  execute: async (sock, msg, args) => {
      try {
         ...
         if ('npmsearch' === 'movie') {
             ...
             try {
                const res = await axios.get(`https://api.jikan.moe/v4/npmsearch?q=${encodeURIComponent(query)}&limit=1`);
                if (res.data && res.data.data && res.data.data.length > 0) {
                    ...
                    extraStr = d.score ? `Score: ${d.score}` : 'No score';
                     ...
                     return sock.sendMessage(msg.key.remoteJid, { text: 'npmsearch not found! 😭' }, { quoted: msg });
                }
             } catch(e) { return sock.sendMessage(msg.key.remoteJid, { text: 'npmsearch API error!' }, { quoted: msg }); }
         } else if ('npmsearch' === 'wikipedia') {
             try {
         ...
         } else if ('npmsearch' === 'githubsearch') {
             try {
