  ...
  execute: async (sock, msg, args) => {
      try {
         ...
         if ('githubsearch' === 'movie') {
             ...
             try {
                const res = await axios.get(`https://api.jikan.moe/v4/githubsearch?q=${encodeURIComponent(query)}&limit=1`);
                if (res.data && res.data.data && res.data.data.length > 0) {
                    ...
                    extraStr = d.score ? `Score: ${d.score}` : 'No score';
                     ...
                     return sock.sendMessage(msg.key.remoteJid, { text: 'githubsearch not found! 😭' }, { quoted: msg });
                }
             } catch(e) { return sock.sendMessage(msg.key.remoteJid, { text: 'githubsearch API error!' }, { quoted: msg }); }
         } else if ('githubsearch' === 'wikipedia') {
             try {
         ...
         } else if ('githubsearch' === 'githubsearch') {
             try {
