  ...
  execute: async (sock, msg, args) => {
      try {
         ...
         if ('wikipedia' === 'movie') {
             ...
             try {
                const res = await axios.get(`https://api.jikan.moe/v4/wikipedia?q=${encodeURIComponent(query)}&limit=1`);
                if (res.data && res.data.data && res.data.data.length > 0) {
                    ...
                    extraStr = d.score ? `Score: ${d.score}` : 'No score';
                     ...
                     return sock.sendMessage(msg.key.remoteJid, { text: 'wikipedia not found! 😭' }, { quoted: msg });
                }
             } catch(e) { return sock.sendMessage(msg.key.remoteJid, { text: 'wikipedia API error!' }, { quoted: msg }); }
         } else if ('wikipedia' === 'wikipedia') {
             try {
         ...
         } else if ('wikipedia' === 'githubsearch') {
             try {
