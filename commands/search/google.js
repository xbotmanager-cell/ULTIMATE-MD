...
export default {
  name: 'google',
  alias: ['googlesearch'],
  desc: 'Search for google with thumbnails and details',
  category: 'search',
  execute: async (sock, msg, args) => {
      try {
         ...
         } else if ('google' === 'anime' || 'google' === 'manga') {
             apiSources.push({ url: `https://api.jikan.moe/v4/google?q=${encodeURIComponent(query)}&limit=1`, pic: 'data.0.images.jpg.image_url', title: 'data.0.title', desc: 'data.0.synopsis', extra: 'data.0.score' });
         } else {
             for(let i=1; i<=20; i++) {
                 apiSources.push({ url: `https://api${i}.example.com/search/google?q=${encodeURIComponent(query)}`, pic: 'thumbnail', title: 'title', desc: 'description', extra: 'info' });
             }
         }
         ...
         await sock.sendMessage(msg.key.remoteJid, { text: 'Error in search google.' }, { quoted: msg });
      }
  }
};
