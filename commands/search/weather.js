...
export default {
  name: 'weather',
  alias: ['weathersearch'],
  desc: 'Search for weather with thumbnails and details',
  category: 'search',
  execute: async (sock, msg, args) => {
      try {
         ...
         } else if ('weather' === 'anime' || 'weather' === 'manga') {
             apiSources.push({ url: `https://api.jikan.moe/v4/weather?q=${encodeURIComponent(query)}&limit=1`, pic: 'data.0.images.jpg.image_url', title: 'data.0.title', desc: 'data.0.synopsis', extra: 'data.0.score' });
         } else {
             for(let i=1; i<=20; i++) {
                 apiSources.push({ url: `https://api${i}.example.com/search/weather?q=${encodeURIComponent(query)}`, pic: 'thumbnail', title: 'title', desc: 'description', extra: 'info' });
             }
         }
         ...
         await sock.sendMessage(msg.key.remoteJid, { text: 'Error in search weather.' }, { quoted: msg });
      }
  }
};
