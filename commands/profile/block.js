...
export default {
  name: 'block',
  desc: 'Profile utility command for block',
  category: 'profile',
  execute: async (sock, msg, args) => {
      ...
      if (!ownerCheck && 'block'.includes('bot')) return sock.sendMessage(msg.key.remoteJid, { text: 'Owner only command!' });

      try {
         const box = createBox(botname, [
            formatLine('ᴀᴄᴛɪᴏɴ', 'block'),
