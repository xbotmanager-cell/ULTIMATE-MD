...
export default {
  name: 'ytaudio',
  alias: ['ytaudiomusic', 'ytaudiodl'],
  desc: 'Music downloader and search for ytaudio',
  category: 'music',
  execute: async (sock, msg, args) => {
      try {
         const query = args.join(' ');
         if (!query) return sock.sendMessage(msg.key.remoteJid, { text: 'Provide a search query or URL for ytaudio!' }, { quoted: msg });

         const apiSources = [];
         
         for(let i=1; i<=40; i++) {
             apiSources.push({ url: `https://api${i}.example.com/music/ytaudio?q=${encodeURIComponent(query)}`, dl: 'download_url', title: 'title', artist: 'artist' });
         }

         // Real alternative free APIs for yt/spotify if applicable
         if ('ytaudio' === 'play' || 'ytaudio' === 'ytmp3') {
         ...
         for (let i = 0; i < apiSources.length; i++) {
            try {
               ...
               if (res.data) {
                  ...
                  if (dlPath) {
                      mediaUrl = dlPath;
                      if (!mediaUrl.startsWith('http') && 'ytaudio' === 'lyrics') {
         ...
         if ('ytaudio' === 'lyrics' && mediaUrl === 'lyrics_found') {
         ...
         const box = createBox(botname, [
            formatLine('ᴛᴏᴏʟ', 'YTAUDIO DOWNLOADER'),
            formatLine('ᴛɪᴛʟᴇ', String(titleStr).substring(0, 20)),
            formatLine('ᴀʀᴛɪsᴛ', String(artistStr).substring(0, 20)),
            formatLine('sᴏᴜʀᴄᴇ', usedApi)
         ]);

         if ('ytaudio' === 'ytmp4' || 'ytaudio' === 'ytvideo') {
         ...
         await sock.sendMessage(msg.key.remoteJid, { text: 'Error in ytaudio downloader.' }, { quoted: msg });
      }
  }
};
