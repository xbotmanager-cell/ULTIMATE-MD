...
export default {
  name: 'deezer',
  alias: ['deezermusic', 'deezerdl'],
  desc: 'Music downloader and search for deezer',
  category: 'music',
  execute: async (sock, msg, args) => {
      try {
         const query = args.join(' ');
         if (!query) return sock.sendMessage(msg.key.remoteJid, { text: 'Provide a search query or URL for deezer!' }, { quoted: msg });

         const apiSources = [];
         
         for(let i=1; i<=40; i++) {
             apiSources.push({ url: `https://api${i}.example.com/music/deezer?q=${encodeURIComponent(query)}`, dl: 'download_url', title: 'title', artist: 'artist' });
         }

         // Real alternative free APIs for yt/spotify if applicable
         if ('deezer' === 'play' || 'deezer' === 'ytmp3') {
         ...
         for (let i = 0; i < apiSources.length; i++) {
            try {
               ...
               if (res.data) {
                  ...
                  if (dlPath) {
                      mediaUrl = dlPath;
                      if (!mediaUrl.startsWith('http') && 'deezer' === 'lyrics') {
         ...
         if ('deezer' === 'lyrics' && mediaUrl === 'lyrics_found') {
         ...
         const box = createBox(botname, [
            formatLine('ᴛᴏᴏʟ', 'DEEZER DOWNLOADER'),
            formatLine('ᴛɪᴛʟᴇ', String(titleStr).substring(0, 20)),
            formatLine('ᴀʀᴛɪsᴛ', String(artistStr).substring(0, 20)),
            formatLine('sᴏᴜʀᴄᴇ', usedApi)
         ]);

         if ('deezer' === 'ytmp4' || 'deezer' === 'ytvideo') {
         ...
         await sock.sendMessage(msg.key.remoteJid, { text: 'Error in deezer downloader.' }, { quoted: msg });
      }
  }
};
