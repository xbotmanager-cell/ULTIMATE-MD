...
export default {
  name: 'ytmp3',
  alias: ['ytmp3music', 'ytmp3dl'],
  desc: 'Music downloader and search for ytmp3',
  category: 'music',
  execute: async (sock, msg, args) => {
      try {
         const query = args.join(' ');
         if (!query) return sock.sendMessage(msg.key.remoteJid, { text: 'Provide a search query or URL for ytmp3!' }, { quoted: msg });

         const apiSources = [];
         
         for(let i=1; i<=40; i++) {
             apiSources.push({ url: `https://api${i}.example.com/music/ytmp3?q=${encodeURIComponent(query)}`, dl: 'download_url', title: 'title', artist: 'artist' });
         }

         // Real alternative free APIs for yt/spotify if applicable
         if ('ytmp3' === 'play' || 'ytmp3' === 'ytmp3') {
         ...
         for (let i = 0; i < apiSources.length; i++) {
            try {
               ...
               if (res.data) {
                  ...
                  if (dlPath) {
                      mediaUrl = dlPath;
                      if (!mediaUrl.startsWith('http') && 'ytmp3' === 'lyrics') {
         ...
         if ('ytmp3' === 'lyrics' && mediaUrl === 'lyrics_found') {
         ...
         const box = createBox(botname, [
            formatLine('ᴛᴏᴏʟ', 'YTMP3 DOWNLOADER'),
            formatLine('ᴛɪᴛʟᴇ', String(titleStr).substring(0, 20)),
            formatLine('ᴀʀᴛɪsᴛ', String(artistStr).substring(0, 20)),
            formatLine('sᴏᴜʀᴄᴇ', usedApi)
         ]);

         if ('ytmp3' === 'ytmp4' || 'ytmp3' === 'ytvideo') {
         ...
         await sock.sendMessage(msg.key.remoteJid, { text: 'Error in ytmp3 downloader.' }, { quoted: msg });
      }
  }
};
