...
export default {
  ...
  execute: async (sock, msg, args) => {
    try {
      ...
      if (!target) return sock.sendMessage(msg.key.remoteJid, { text: 'Whom to promote?' }, { quoted: msg });
      
      await sock.groupParticipantsUpdate(msg.key.remoteJid, [target], 'promote');
      const box = createBox('ᴍᴇᴍʙᴇʀ ᴘʀᴏᴍᴏᴛᴇᴅ', [
        formatLine('ᴜsᴇʀ', `@${target.split('@')[0]}`),
        formatLine('sᴛᴀᴛᴜs', 'Congrats you just got the crown!')
      ]);
