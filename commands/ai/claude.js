...
export default {
    ...
    execute: async (sock, msg, args) => {
        try {
            ...
            const systemPrompt = `You are Claude, an AI assistant created by Anthropic. Be helpful, honest, and harmless. Answer directly and concisely.`;
            ...
            if (!responseText) {
                try {
                    ...
                    if (res.data && res.data.message) {
                        ...
                        responseText = `(CLAUDE API Offline Mode): I read you, but I'm currently without my API key or primary network connection. You said: ${fullQuery}`;
                    }
                } catch(e) {
                    responseText = `(CLAUDE): Sorry, my network is heavily congested right now!`;
                }
            }
            
            memory[jid].push({ role: 'User', content: fullQuery });
            memory[jid].push({ role: 'CLAUDE', content: responseText });
