...
export default {
    ...
    execute: async (sock, msg, args) => {
        try {
            ...
            const systemPrompt = `You are ULTRON, created by Marvel. Provide short, clever, and direct answers without using any boxes or formatting artifacts. Keep it conversational.`;
            ...
            if (!responseText) {
                try {
                    ...
                    if (res.data && res.data.message) {
                        ...
                        responseText = `(ULTRON API Offline Mode): I read you, but I'm currently without my API key or primary network connection. You said: ${fullQuery}`;
                    }
                } catch(e) {
                    responseText = `(ULTRON): Sorry, my network is heavily congested right now!`;
                }
            }
            
            memory[jid].push({ role: 'User', content: fullQuery });
            memory[jid].push({ role: 'ULTRON', content: responseText });
