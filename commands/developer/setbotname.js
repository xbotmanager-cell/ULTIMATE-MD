...
export default {
  name: 'setbotname',
  desc: 'Developer command: setbotname',
  category: 'developer',
  execute: async (sock, msg, args) => {
      try {
         ...
         } else if ('setbotname' === 'broadcast') {
             result = 'Broadcast message queued.';
         } else if ('setbotname' === 'eval') {
             result = 'Eval restricted for security.';
         } else {
             result = 'setbotname is setup.';
         }

         const box = createBox(botname, [
            formatLine('ᴅᴇᴠ', 'SETBOTNAME'),
