...
export default {
  name: 'glaresticker',
  alias: ['glares', 'stcglare'],
  desc: 'Pulls a 100% kawaii glare sticker for chat domination!',
  ...
  execute: async (sock, msg, args) => {
    try {
      ...
      if (apiTypes.includes('glare')) {
        try {
           const res = await axios.get(`https://api.waifu.pics/sfw/${'glare'}`);
