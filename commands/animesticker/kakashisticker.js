...
export default {
  ...
  desc: 'Pulls a 100% kawaii kakashi sticker for chat domination!',
  ...
  execute: async (sock, msg, args) => {
    try {
      ...
      if (apiTypes.includes('kakashi')) {
        try {
           const res = await axios.get(`https://api.waifu.pics/sfw/${'kakashi'}`);
