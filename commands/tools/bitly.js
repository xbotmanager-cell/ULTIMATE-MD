...
export default {
  ...
  execute: async (sock, msg, args) => {
      try {
         ...
         } else if ('bitly' === 'qrcreate' || 'bitly' === 'qrcode') {
         ...
         } else if ('bitly' === 'tinyurl' || 'bitly' === 'shorturl') {
         ...
         } else if ('bitly' === 'password') {
             result = Math.random().toString(36).slice(-10) + Math.random().toString(36).toUpperCase().slice(-5) + '@!';
         } else {
             result = `bitly tool result: ${input || 'success'}`;
         }

         await sock.sendMessage(msg.key.remoteJid, { text: result }, { quoted: msg });
      } catch (e) {
         await sock.sendMessage(msg.key.remoteJid, { text: 'Yikes, executing bitly went completely sideways 😂.' }, { quoted: msg });
      }
  }
};
