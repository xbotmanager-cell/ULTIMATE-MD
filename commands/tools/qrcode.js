...
export default {
  ...
  execute: async (sock, msg, args) => {
      try {
         ...
         } else if ('qrcode' === 'qrcreate' || 'qrcode' === 'qrcode') {
         ...
         } else if ('qrcode' === 'tinyurl' || 'qrcode' === 'shorturl') {
         ...
         } else if ('qrcode' === 'password') {
             result = Math.random().toString(36).slice(-10) + Math.random().toString(36).toUpperCase().slice(-5) + '@!';
         } else {
             result = `qrcode tool result: ${input || 'success'}`;
         }

         await sock.sendMessage(msg.key.remoteJid, { text: result }, { quoted: msg });
      } catch (e) {
         await sock.sendMessage(msg.key.remoteJid, { text: 'Yikes, executing qrcode went completely sideways 😂.' }, { quoted: msg });
      }
  }
};
