...
export default {
  ...
  execute: async (sock, msg, args) => {
      try {
         ...
         } else if ('tinyurl' === 'qrcreate' || 'tinyurl' === 'qrcode') {
         ...
         } else if ('tinyurl' === 'tinyurl' || 'tinyurl' === 'shorturl') {
         ...
         } else if ('tinyurl' === 'password') {
             result = Math.random().toString(36).slice(-10) + Math.random().toString(36).toUpperCase().slice(-5) + '@!';
         } else {
             result = `tinyurl tool result: ${input || 'success'}`;
         }

         await sock.sendMessage(msg.key.remoteJid, { text: result }, { quoted: msg });
      } catch (e) {
         await sock.sendMessage(msg.key.remoteJid, { text: 'Yikes, executing tinyurl went completely sideways 😂.' }, { quoted: msg });
      }
  }
};
