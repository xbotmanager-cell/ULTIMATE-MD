...
export default {
  name: 'qrcreate',
  desc: 'Utility tool for qrcreate',
  category: 'tools',
  react: '🔧',
  execute: async (sock, msg, args) => {
      try {
         ...
         } else if ('qrcreate' === 'qrcreate' || 'qrcreate' === 'qrcode') {
         ...
         } else if ('qrcreate' === 'tinyurl' || 'qrcreate' === 'shorturl') {
         ...
         } else if ('qrcreate' === 'password') {
             result = Math.random().toString(36).slice(-10) + Math.random().toString(36).toUpperCase().slice(-5) + '@!';
         } else {
             result = `qrcreate tool result: ${input || 'success'}`;
         }

         await sock.sendMessage(msg.key.remoteJid, { text: result }, { quoted: msg });
      } catch (e) {
         await sock.sendMessage(msg.key.remoteJid, { text: 'Yikes, executing qrcreate went completely sideways 😂.' }, { quoted: msg });
      }
  }
};
