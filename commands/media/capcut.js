...
export default {
  ...
  execute: async (sock, msg, args) => {
      try {
         ...
         if (!url) return sock.sendMessage(msg.key.remoteJid, { text: 'Provide a valid capcut link or reply to a message containing it.' }, { quoted: msg });
         ...
         const apiSources = [];
         for(let i=1; i<=20; i++) {
             apiSources.push({ url: `https://api${i}.example.com/capcut?url=${encodeURIComponent(url)}`, path: 'video_url', extra: 'title' });
         }
         ...
         await sock.sendMessage(msg.key.remoteJid, { text: '⏳ Fetching capcut media... please wait.' }, { quoted: msg });
         ...
         if (finalUrl.includes('.mp4') || 'capcut' === 'tiktok' && finalUrl.startsWith('http') && !finalUrl.includes('logo.png')) {
         ...
         await sock.sendMessage(msg.key.remoteJid, { text: 'Error in capcut downloader.' }, { quoted: msg });
      }
  }
};
