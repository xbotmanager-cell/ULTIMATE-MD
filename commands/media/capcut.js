...
export default {
  name: 'capcut',
  alias: ['capcutdl', 'capcutdown'],
  desc: 'Download media from CAPCUT',
  category: 'media',
  execute: async (sock, msg, args) => {
      try {
         ...
         if (!url) return sock.sendMessage(msg.key.remoteJid, { text: 'Bruh, provide a valid capcut link or reply to one!' }, { quoted: msg });
         ...
         await sock.sendMessage(msg.key.remoteJid, { text: '⏳ Fetching capcut media... hang tight.' }, { quoted: msg });
         ...
         } else if ('capcut' === 'instagram' || 'capcut' === 'facebook' || 'capcut' === 'twitter') {
            try {
               const res = await axios.get(`https://api.akuari.my.id/downloader/${'capcut' === 'instagram' ? 'igdl' : 'capcut'}?link=${encodeURIComponent(url)}`);
               if (res.data?.result?.url || res.data?.url?.[0]?.url) {
                   finalUrl = res.data.result?.url || res.data.url[0].url;
               } else { throw new Error('Not found'); }
            } catch(e) { return sock.sendMessage(msg.key.remoteJid, { text: 'CAPCUT download failed! Try another link. 😭' }, { quoted: msg }); }
         } else {
             return sock.sendMessage(msg.key.remoteJid, { text: 'Downloading from CAPCUT is temporarily unavailable!' }, { quoted: msg });
         }

         const botname = get('botname') || 'ULTIMATE-MD';
         const box = createBox(botname, [
            formatLine('ᴛᴏᴏʟ', 'CAPCUT DOWNLOADER'),
         ...
         await sock.sendMessage(msg.key.remoteJid, { text: 'Downloading capcut just crashed my tiny server brain 😭.' }, { quoted: msg });
      }
  }
};