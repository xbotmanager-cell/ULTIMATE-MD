...
export default {
  ...
  execute: async (sock, msg, args) => {
      try {
         ...
         if (!url) return sock.sendMessage(msg.key.remoteJid, { text: 'Bruh, provide a valid twitter link or reply to one!' }, { quoted: msg });
         ...
         await sock.sendMessage(msg.key.remoteJid, { text: '⏳ Fetching twitter media... hang tight.' }, { quoted: msg });
         ...
         } else if ('twitter' === 'instagram' || 'twitter' === 'facebook' || 'twitter' === 'twitter') {
            try {
               const res = await axios.get(`https://api.akuari.my.id/downloader/${'twitter' === 'instagram' ? 'igdl' : 'twitter'}?link=${encodeURIComponent(url)}`);
               if (res.data?.result?.url || res.data?.url?.[0]?.url) {
                   finalUrl = res.data.result?.url || res.data.url[0].url;
               } else { throw new Error('Not found'); }
            } catch(e) { return sock.sendMessage(msg.key.remoteJid, { text: 'TWITTER download failed! Try another link. 😭' }, { quoted: msg }); }
         } else {
             return sock.sendMessage(msg.key.remoteJid, { text: 'Downloading from TWITTER is temporarily unavailable!' }, { quoted: msg });
         }
         ...
         await sock.sendMessage(msg.key.remoteJid, { text: 'Downloading twitter just crashed my tiny server brain 😭.' }, { quoted: msg });
      }
  }
};