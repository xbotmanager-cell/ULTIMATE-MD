...
export default {
  name: 'pinterest',
  alias: ['pinterestdl', 'pinterestdown'],
  ...
  execute: async (sock, msg, args) => {
      try {
         ...
         if (!url) return sock.sendMessage(msg.key.remoteJid, { text: 'Bruh, provide a valid pinterest link or reply to one!' }, { quoted: msg });
         ...
         await sock.sendMessage(msg.key.remoteJid, { text: '⏳ Fetching pinterest media... hang tight.' }, { quoted: msg });
         ...
         } else if ('pinterest' === 'instagram' || 'pinterest' === 'facebook' || 'pinterest' === 'twitter') {
            try {
               const res = await axios.get(`https://api.akuari.my.id/downloader/${'pinterest' === 'instagram' ? 'igdl' : 'pinterest'}?link=${encodeURIComponent(url)}`);
               if (res.data?.result?.url || res.data?.url?.[0]?.url) {
                   finalUrl = res.data.result?.url || res.data.url[0].url;
               } else { throw new Error('Not found'); }
            } catch(e) { return sock.sendMessage(msg.key.remoteJid, { text: 'PINTEREST download failed! Try another link. 😭' }, { quoted: msg }); }
         } else {
             return sock.sendMessage(msg.key.remoteJid, { text: 'Downloading from PINTEREST is temporarily unavailable!' }, { quoted: msg });
         }

         const botname = get('botname') || 'ULTIMATE-MD';
         const box = createBox(botname, [
            formatLine('ᴛᴏᴏʟ', 'PINTEREST DOWNLOADER'),
         ...
         await sock.sendMessage(msg.key.remoteJid, { text: 'Downloading pinterest just crashed my tiny server brain 😭.' }, { quoted: msg });
      }
  }
};