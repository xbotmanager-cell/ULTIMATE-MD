...
export default {
  ...
  execute: async (sock, msg, args) => {
      try {
         ...
         if (!url) return sock.sendMessage(msg.key.remoteJid, { text: 'Bruh, provide a valid facebook link or reply to one!' }, { quoted: msg });
         ...
         await sock.sendMessage(msg.key.remoteJid, { text: '⏳ Fetching facebook media... hang tight.' }, { quoted: msg });
         ...
         } else if ('facebook' === 'instagram' || 'facebook' === 'facebook' || 'facebook' === 'twitter') {
            try {
               const res = await axios.get(`https://api.akuari.my.id/downloader/${'facebook' === 'instagram' ? 'igdl' : 'facebook'}?link=${encodeURIComponent(url)}`);
               if (res.data?.result?.url || res.data?.url?.[0]?.url) {
                   finalUrl = res.data.result?.url || res.data.url[0].url;
               } else { throw new Error('Not found'); }
            } catch(e) { return sock.sendMessage(msg.key.remoteJid, { text: 'FACEBOOK download failed! Try another link. 😭' }, { quoted: msg }); }
         } else {
             return sock.sendMessage(msg.key.remoteJid, { text: 'Downloading from FACEBOOK is temporarily unavailable!' }, { quoted: msg });
         }
         ...
         await sock.sendMessage(msg.key.remoteJid, { text: 'Downloading facebook just crashed my tiny server brain 😭.' }, { quoted: msg });
      }
  }
};