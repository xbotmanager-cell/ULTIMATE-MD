...
export default {
  name: 'instagram',
  alias: ['instagramdl', 'instagramdown'],
  ...
  execute: async (sock, msg, args) => {
      try {
         ...
         if (!url) return sock.sendMessage(msg.key.remoteJid, { text: 'Bruh, provide a valid instagram link or reply to one!' }, { quoted: msg });
         ...
         await sock.sendMessage(msg.key.remoteJid, { text: '⏳ Fetching instagram media... hang tight.' }, { quoted: msg });
         ...
         } else if ('instagram' === 'instagram' || 'instagram' === 'facebook' || 'instagram' === 'twitter') {
            try {
               const res = await axios.get(`https://api.akuari.my.id/downloader/${'instagram' === 'instagram' ? 'igdl' : 'instagram'}?link=${encodeURIComponent(url)}`);
               if (res.data?.result?.url || res.data?.url?.[0]?.url) {
                   finalUrl = res.data.result?.url || res.data.url[0].url;
               } else { throw new Error('Not found'); }
            } catch(e) { return sock.sendMessage(msg.key.remoteJid, { text: 'INSTAGRAM download failed! Try another link. 😭' }, { quoted: msg }); }
         } else {
             return sock.sendMessage(msg.key.remoteJid, { text: 'Downloading from INSTAGRAM is temporarily unavailable!' }, { quoted: msg });
         }

         const botname = get('botname') || 'ULTIMATE-MD';
         const box = createBox(botname, [
            formatLine('ᴛᴏᴏʟ', 'INSTAGRAM DOWNLOADER'),
         ...
         await sock.sendMessage(msg.key.remoteJid, { text: 'Downloading instagram just crashed my tiny server brain 😭.' }, { quoted: msg });
      }
  }
};