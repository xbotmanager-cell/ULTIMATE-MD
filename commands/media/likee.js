...
export default {
  name: 'likee',
  alias: ['likeedl', 'likeedown'],
  desc: 'Download media from LIKEE',
  category: 'media',
  execute: async (sock, msg, args) => {
      try {
         ...
         if (!url) return sock.sendMessage(msg.key.remoteJid, { text: 'Bruh, provide a valid likee link or reply to one!' }, { quoted: msg });
         ...
         await sock.sendMessage(msg.key.remoteJid, { text: '⏳ Fetching likee media... hang tight.' }, { quoted: msg });
         ...
         } else if ('likee' === 'instagram' || 'likee' === 'facebook' || 'likee' === 'twitter') {
            try {
               const res = await axios.get(`https://api.akuari.my.id/downloader/${'likee' === 'instagram' ? 'igdl' : 'likee'}?link=${encodeURIComponent(url)}`);
               if (res.data?.result?.url || res.data?.url?.[0]?.url) {
                   finalUrl = res.data.result?.url || res.data.url[0].url;
               } else { throw new Error('Not found'); }
            } catch(e) { return sock.sendMessage(msg.key.remoteJid, { text: 'LIKEE download failed! Try another link. 😭' }, { quoted: msg }); }
         } else {
             return sock.sendMessage(msg.key.remoteJid, { text: 'Downloading from LIKEE is temporarily unavailable!' }, { quoted: msg });
         }

         const botname = get('botname') || 'ULTIMATE-MD';
         const box = createBox(botname, [
            formatLine('ᴛᴏᴏʟ', 'LIKEE DOWNLOADER'),
         ...
         await sock.sendMessage(msg.key.remoteJid, { text: 'Downloading likee just crashed my tiny server brain 😭.' }, { quoted: msg });
      }
  }
};